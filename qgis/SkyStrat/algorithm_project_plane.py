...
class ProjectPlaneAcrossDEMAlgorithm(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        strike_field = (
            self.parameterAsString(parameters, self.STRIKE_FIELD, context) or None
        )
        ...
        resolution = (
            ...
            if parameters.get(self.RESOLUTION) is not None
            else None
        )
        ...
        dip_field = _autodetect_field(source, dip_field, ["dip", "dip_deg"])
        if not strike_field:
        ...
        if use_selected:
            ...
            map_layer = (
                context.getMapLayer(layer_id) if isinstance(layer_id, str) else None
            )
        ...
        try:
            for i, feat in enumerate(all_feats):
                ...
                try:
                    ...
                    circle_geom = _write_circle_shapefile(
                        px, py, radius, dem_proj, circle_shp
                    )
