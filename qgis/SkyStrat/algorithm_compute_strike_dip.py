...
class ComputeStrikeDipAlgorithm(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        triplet_field = (
            self.parameterAsString(parameters, self.TRIPLET_FIELD, context) or None
        )
        ...
        if QgsWkbTypes.geometryType(source.wkbType()) != QgsWkbTypes.PointGeometry:
            raise QgsProcessingException(
                "The Point Triplets Layer must be a point feature layer."
            )
        ...
        if triplet_field:
            ...
            for feat in source.getFeatures():
                ...
                if tid is None:
                    feedback.pushWarning(
                        f"Feature FID {feat.id()} has null TripletID; skipping."
                    )
                    continue
            ...
            groups = [
                sorted(feats, key=lambda f: f.id()) for feats in triplet_map.values()
            ]
        else:
        ...
        for i, trip in enumerate(groups):
            ...
            try:
                ...
                for j, feat in enumerate(trip):
                    ...
                    feedback.pushInfo(
                        f"XYZ for point {j} in triplet {i}: ({pt.x():.2f}, {pt.y():.2f}, {z:.2f})"
                    )
                ...
                centroid_x = (
                    items_sorted[1][1] + items_sorted[2][1] + items_sorted[0][1]
                ) / 3.0
                centroid_y = (
                    items_sorted[1][2] + items_sorted[2][2] + items_sorted[0][2]
                ) / 3.0

                nearest_idx = min(
                    range(3),
                    key=lambda j: math.hypot(
                        items_sorted[j][1] - centroid_x, items_sorted[j][2] - centroid_y
                    ),
                )
                ...
                if math.isnan(strike) or math.isnan(dip):
                    ...
                    continue
                feedback.pushInfo(
                    f"Strike/dip {j} in triplet {i}: {strike:.2f}/{dip:.2f}"
                )
