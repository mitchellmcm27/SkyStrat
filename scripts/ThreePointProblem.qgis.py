...
def _stable_hash_latlons(lats_lons):
    """
    Order-independent SHA-256 hash of three (lat, lon) pairs.
    Coordinates rounded to 6 decimal places (~0.1 m) for robustness.
    """
...
def _point_to_wgs84(x, y, src_crs, transform_context):
    """
    Project a coordinate pair from src_crs to WGS84.
    Returns (lat, lon). Replaces arcpy geometry.projectAs(WGS84).
    """
    wgs84 = QgsCoordinateReferenceSystem('EPSG:4326')
    xform = QgsCoordinateTransform(src_crs, wgs84, transform_context)
...
class ComputeStrikeDipAlgorithm(QgsProcessingAlgorithm):
    ...
    def shortHelpString(self):
        return (
            ...
            'SRC_HASH (SHA-256 of WGS84 coordinates) is stored per output point '
            'for incremental deduplication across runs.'
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        out_fields.append(QgsField('SRC_LAT1', QVariant.Double))
        out_fields.append(QgsField('SRC_LAT2', QVariant.Double))
        out_fields.append(QgsField('SRC_LAT3', QVariant.Double))
        out_fields.append(QgsField('SRC_LON1', QVariant.Double))
        out_fields.append(QgsField('SRC_LON2', QVariant.Double))
        out_fields.append(QgsField('SRC_LON3', QVariant.Double))
        out_fields.append(QgsField('SRC_HASH', QVariant.String, len=64))
        ...
        inserted = 0
        skipped = 0
        seen_hashes = set()   # deduplication within this run
        ...
        for i, trip in enumerate(groups):
            ...
            try:
                ...
                # Compute WGS84 lat/lon for stable hash (order-independent)
                latlons = [
                    _point_to_wgs84(x, y, src_crs, transform_context)
                ...
                # Compute WGS84 lat/lon per FID for output fields
                latlons_sorted = [
                    _point_to_wgs84(x, y, src_crs, transform_context)
                    for _, x, y, _ in items_sorted
                ]
                ...
                out_feat['SRC_LAT1'] = latlons_sorted[0][0]
                out_feat['SRC_LAT2'] = latlons_sorted[1][0]
                out_feat['SRC_LAT3'] = latlons_sorted[2][0]
                out_feat['SRC_LON1'] = latlons_sorted[0][1]
                out_feat['SRC_LON2'] = latlons_sorted[1][1]
                out_feat['SRC_LON3'] = latlons_sorted[2][1]
                out_feat['SRC_HASH'] = src_hash
